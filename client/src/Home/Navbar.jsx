...
const Navbar = () => {
    ...
    const isAdmin = auth.user?.role.code === 'administrator'
        ...
        <header className={color ? 'header_wrapper header-scrolled' : 'header_wrapper'}>
            <nav className="navbar navbar-expand-lg fixed-top">
                <div className="container-fluid mx-3">
                    <Link to='/'>
                        <img src={logo2} style={{ width: '130px' }} />
                    </Link>
                    ...
                    <div className="collapse navbar-collapse justify-content-end" id="navbarNav">
                        <ul className="navbar-nav menu-navbar-nav">
                            ...
                            <Link to={isAdmin ? '/dashboard/admin/allbrands' : '/brands'} style={{ textDecoration: 'none' }}>
                                <li className="nav-item">
                                    <a className="nav-link " aria-current="page">Brands</a>
                                </li>
                            </Link>
                            <Link to={isAdmin? 'dashboard/admin/cars':'/cars'} style={{ textDecoration: 'none' }}>
                                <li className="nav-item">
                        ...
                        {!auth.user ? (<ul className='mt-2 text-center'>
                            ...
                            <Link to={`/dashboard/${auth?.user?.role.code === 'administrator' ? "admin" : "user"}`} style={{ textDecoration: 'none' }} className="nav-item text-center">
