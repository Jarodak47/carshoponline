...
const Cart = () => {
    ...
    const totalPrice = () => {
        ...
        try {
            ...
            if (cart && Array.isArray(cart)) {
                cart.forEach((item) => {
                    const po = item.price;
                    total += parseInt(po, 10);
                });
            }
                
            return total.toLocaleString("en-US", { // Utilisation de "en-IN" pour le format indien
                style: "currency",
                currency: "USD",
            });
        } catch (error) {
    ...
    const getToken = async () => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/payments/token`);
            setClientToken(data);
        } catch (error) {
            console.log(error);
        }
    ...
    const handlePayment = async () => {
        try {
            ...
            const { data } = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/payments/create`, {
                nonce,
                cart
            });
            setLoading(false);
