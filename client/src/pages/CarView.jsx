...
import Hover3DEffect from '../common/Hover3DEffect';
...
const CarView = () => {
    ...
    const getCar = async () => {
        try {
            ...
            setCar(data);
            getRelatedCar(data?.uuid, data?.brand.uuid);
            setLoading(false);
        } catch (err) {
            console.log(err);
            setLoading(true);
    ...
    const getRelatedCar = async (cid, bid) => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/vehicles/brand/${cid}/${bid}`)
            setRelatedCar(data?.vehicles)
        } catch (err) {
            console.log(err)
        }
    }
    ...
    const updatedAt = new Date(car.date_modified).toLocaleString();
